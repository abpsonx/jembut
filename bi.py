...
def key():
    ...
    try:
        ses = requests.Session()
        asu = ses.get("https://app.cryptolens.io/api/key/Activate?token=WyI3NDQ4NDYyMSIsIjU0U3QxSDQ5WGw3N1ZKVjZnV2lHNHk3cFFVOVZjcHYrd3NObmJZK24iXQ==&ProductId=24000&Key=%s&Sign=True"%(key)).json()['licenseKey']['key']
...
def cek():
    try:
        ...
        licen()
    try:
        lopi = requests.get("https://app.cryptolens.io/api/key/Activate?token=WyI3NDQ4NDYyMSIsIjU0U3QxSDQ5WGw3N1ZKVjZnV2lHNHk3cFFVOVZjcHYrd3NObmJZK24iXQ==&ProductId=24000&Key=%s"%(x)).json()['licenseKey']['key']
        login()
    except KeyError:
	...
	if cokis in [""," "]:login()
		...
		try:
			...
			if "EAAG" in coli:
				...
				open(".token.txt","w").write(coli)
				open(".cok.txt","w").write(cokis)
		except AttributeError:print(f"\n {M}•{K}• {N}Cookie invalid");time.sleep(3);login()
		except UnboundLocalError:print(f"\n {M}•{K}• {N}cokie invalid");time.sleep(3);login()
		except requests.exceptions.TooManyRedirects:print(f"\n {M}•{K}• {N} Masukan cokie anda dengan benar");time.sleep(3);login()
...
def menu(my_name,my_id):
    try:
        ...
        met = ses.get('https://app.cryptolens.io/api/key/Activate?token=WyI3NDQ4NDYyMSIsIjU0U3QxSDQ5WGw3N1ZKVjZnV2lHNHk3cFFVOVZjcHYrd3NObmJZK24iXQ==&ProductId=24000&Key='+lisen).json()
        men = met['licenseKey']
        key = men['key'][0:11]
